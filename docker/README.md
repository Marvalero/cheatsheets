...
![VMs Architecture](./vm_architecture.png)
...
![Containers Architecture](./container_architecture.png)
...
The Snapshotter (or storage-drivers) is responsible for stacking layers and presenting them as a single unified filesystem. Examples of storage-drivers on Linux include: AUFS, overlay2, devicemapper, btrfs and zfs (each one is based on a Linux filesystem or block-device technology).
 ...
 * ENTRYPOINT: set the main application that the container should run. This is also added as metadata.
 ...
 You can squash an image to produce a single layer. This is a good practice for images taht will be used in FROM. Use `--squash` to build the image
